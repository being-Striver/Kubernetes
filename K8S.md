...
In cluster, we have one master node and multiple docker nodes.The master node will give instructions to all remaining docker nodes about containers to run.If any docker node fails,master node automatically provision new node.
...
So you don't have to login into master node and tell it to run the container.you can connect it by some client.you give information to master node that I want to run so and so conatiners and it is going to take actions based on the requirements.So you have in master node which is also called control panel.you have following service in master node:
  1. API server
  2. Scheduler
...
---------------------------------
 kubelet : 
  ...
  - this is going to listen K8s master request or commands.When the scheduler decides that this worker node is going to run the container, its going to assign the responsibility to kubelet so kubelet is going fetch your image and run container from it.so its going to do heavy lifting.
...
**NOTE**: Containers enclosed in pod.

# What is relation between container and pod?
-------------------------------------------------------------------------------------------------------
It's the same relation a VM has with the process running inside it.So let's say a Tomcat process is running in a VM so VM is going to provide all the resource to the process.Process just uses it.Similar way pod is going to provide all the resources to a container.The container will be running inside the pod.Container will be like process and pod will be like VM.
...
-------------------------------------------------------------------
K8s can use different runtime environement like docker, rocket, CRI.If you don't have pods,there will be no abstraction.
...
Now we gonna see how to setup K8s cluster using miniKube and Cops.
...
-Open powershell and run `minikube start`

- when you put above command and hit enter , it is going to launch one single VM on your virtual box so you should have already oracle virtual box.thats it. cluster will be up and running.
...
- when data become more complex, you typically ending up transacting in data format like Json or YAML.It's easy to install and get started with ETCD.
...
The default client comes with ETCD control client.ETCD control client is command line client for ETCD.
...
K8s cluster can have multiple namespaces. These namespaces will be used to set various kind of securities, quotas and few other things to your resources.
...
Pods:
------------------
A pod is a basic execution unit of a k8s application- the smallest and simplest unit in the K8s objects model that you create or deploy. A pod represents processes running on your cluster.
...
Overview of K8s objects:
-------------------------
K8s objects is basically a record of intent that you pass on the K8s cluster. Once you create the object, K8s system will constantly work to ensure that object exists.

  Approach to configure an object:
  1. first way through kubectl commands
  2. second approach is through configuration file written in yaml.
